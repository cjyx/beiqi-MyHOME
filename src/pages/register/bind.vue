      ...
      <div class="form-content">
        ...
        <!--<div class="form-group">-->
          <!--<div>-->
            ...
            <!--<span>点击拍摄/上传</span>-->
          <!--</div>-->
          <!--&lt;!&ndash;<i class="iconfont icon-car-no"></i>&ndash;&gt;-->
          <!--<img :src="cardImg">-->
        <!--</div>-->
...
<script>
  ...
  export default {
    ...
    mounted(){
      this.native.grabHandler('register.bind');
...
//      失去焦点验证vin
      ...
      submit(){
        if(this.carCanAdd==false){
          this.toast('您已提交过车辆认证，请等待审核');
          return;
        }
        ...
        this.native.apiHandler('XS-VEH-CLBD', this.form, (res) => {
          this.native.grabHandler('register.bind.submit');
          let response = this.convert(res);
          if (response.body.isSuccessful == true) {
            this.sub = false;
            var resultData = response.body.resultData;
            if (resultData.isSuccess) {
              ...
              this.$router.push({name: 'bindPerfect', query: {id: resultData.id, vin: this.form.vin}});
            } else {
