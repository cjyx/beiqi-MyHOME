<template>
  <div class="app">
    ...
    <div class="main-content">
      <div class="srach-btn">
        <input type="text" placeholder="请输入车友会名称/城市" v-model="searchContent">
        ...
        <mt-loadmore :autoFill="false" :bottom-method="loadBottom" @bottom-status-change="handleBottomChange" :bottom-all-loaded="allLoaded" ref="loadmore">
          <ul class="page-loadmore-list group-list">
            <li v-if="dataContent.auditType==0 || dataContent.auditType==1">
              <a @click="detial('clubSearchJoin',dataContent.id,dataContent)">
                ...
                <div class="content">
                  ...
                  <div class="club-list">
                    ...
                    <p>车友会成立时间：{{dataContent.createTime}}</p>
                  </div>
                </div>
            ...
            <li v-for="(item,index) in list" :key="index">
              <a @click="detial('club',item.id,dataContent)">
                <img :src="item.picUrl">
                <div class="content">
                  <h3>{{item.name}}<span class="red" v-if="item.city!=null">（{{item.city}}）</span></h3>
                  <div class="club-list">
                    <p>车友会人数：{{item.total}}</p>
                    <p>车友会成立时间：{{item.createTime}}</p>
                  </div>
                </div>
                <button class="btn" v-if="dataContent.auditType!=0 && dataContent.auditType!=1" @click.stop="join(item.id)">申请加入</button>
              </a>
...
//      跳转详情
      detial(name,id,type){
        this.native.grabHandler('home.club.search.detail.'+id);
...
//      申请加入
      join(id){
        if(this.token==''){
          ...
          this.native.grabHandler('home.club.search.join');
...
//      搜索车友会
      search(){
        ...
        this.init(1);
        this.native.grabHandler('home.club.search.searchBtn');
      }
    },
    mounted() {
      if (this.token != '') {
      ...
      this.native.grabHandler('home.club.search');
    }
  }
</script>
