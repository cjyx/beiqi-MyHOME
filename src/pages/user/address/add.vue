    ...
    <div class="main-content">
      <div class="form-content">
        <div class="form-group">
          ...
          <input type="text" maxlength="20" v-model="form.name" placeholder="请输入姓名">
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          ...
          <div class="option square">
            <input type="radio" name="status" value="0"  v-model="form.status" checked>
            <span></span>
            <label>否</label>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    mounted(){
      this.native.grabHandler('user.information.address.addressAdd');
      this.tag()
    },
    methods: {
      //标签列表
...
//        this.form.status = this.form.status ? '1' : '0';
        this.native.apiHandler('XS-VEH-DZBJ', this.form, (res) => {
          this.native.grabHandler('user.information.address.addressAdd.submit');
          let response = this.convert(res);
          if (response.body.isSuccessful == true) {
